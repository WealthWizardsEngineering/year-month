...
test('monthNum constructor and toString()', t => {
  const scenarios = [
    [0, '1970-01'],
    [1, '1970-02'],
    [11, '1970-12'],
  ...
  scenarios.forEach(([monthNum, expected]) => {
    t.test('1', assert => {
      assert.plan(1);
      const yearMonth = new YearMonth(monthNum);

      assert.equals(yearMonth.toString(), expected, `Month number ${monthNum} is ${expected}`);
    });
  });
});

test('parse constructor and toString()', t => {
  const scenarios = [
    '1970-01',
    '1970-02',
    '1970-12',
